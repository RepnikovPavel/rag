#!/usr/bin/env bash
# download_qwen3_vl_full.sh - Скачивание Qwen/Qwen3-VL-235B-A22B-Instruct
# Основано на точном списке файлов (96 шард_weights)

set -euo pipefail

# --- НАСТРОЙКИ ---
MODEL_ID="Qwen/Qwen3-VL-235B-A22B-Instruct"
# Папка, куда скачивать (убедитесь, что есть ~480 ГБ места!)
TARGET_DIR="/mnt/nvme/huggingface/Qwen3-VL-235B-A22B-Instruct"

echo "🚀 Скачиваем $MODEL_ID"
echo "📂 Папка назначения: $TARGET_DIR"
echo "⚠️  Обратите внимание: Модель состоит из 96 частей, общий размер ~470 ГБ."

# Создаем папку
mkdir -p "$TARGET_DIR"
cd "$TARGET_DIR"

# Функция для скачивания с повторными попытками
do_download() {
    local url="$1"
    local filename="$2"
    
    # Если файл уже есть и имеет нормальный размер, пропускаем проверку сервером (чтобы быстрее)
    if [[ -f "$filename" && ! -s "$filename" ]]; then
        echo "⚠️  Файл $filename существует, но пустой. Удаляю..."
        rm "$filename"
    fi

    echo "📥 Скачиваю: $filename"
    
    # Wget с поддержкой докачки (-c), таймаутом и 3 попытками
    wget -c --timeout=3600 --tries=3 --continue \
         --user-agent="Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" \
         -O "$filename" "$url" || {
        echo "❌ Ошибка скачивания $filename. Попробуйте запустить скрипт снова."
        return 1
    }
}

# ---------------------------------------------------------
# ЧАСТЬ 1: Скачивание весов (96 файлов safetensors)
# ---------------------------------------------------------
echo "========================================"
echo "⬇️  ЧАСТЬ 1: Веса модели (96 файлов)..."
echo "========================================"

# Цикл от 001 до 096 с добавлением нулей (padding)
for i in {001..096}; do
    # Формируем имя файла: model-00001-of-00096.safetensors
    FILE_NAME=$(printf "model-%05d-of-00096.safetensors" "$i")
    FILE_URL="https://huggingface.co/$MODEL_ID/resolve/main/$FILE_NAME"
    
    do_download "$FILE_URL" "$FILE_NAME"
done

echo "✅ Часть 1 завершена: Все веса скачаны."

# ---------------------------------------------------------
# ЧАСТЬ 2: Скачивание конфигурационных файлов
# ---------------------------------------------------------
echo "========================================"
echo "⬇️  ЧАСТЬ 2: Конфигурации и Токенизатор..."
echo "========================================"

# Список всех нужных файлов (взято из вашего сообщения)
CONFIG_FILES=(
    "config.json"
    "generation_config.json"
    "model.safetensors.index.json"
    "preprocessor_config.json"
    "tokenizer.json"
    "tokenizer_config.json"
    "video_preprocessor_config.json"
    "vocab.json"
    "merges.txt"
    "chat_template.json" # Важно для чат-моделей
)

for file in "${CONFIG_FILES[@]}"; do
    FILE_URL="https://huggingface.co/$MODEL_ID/resolve/main/$file"
    do_download "$FILE_URL" "$file"
done

echo "✅ Часть 2 завершена: Конфигурации скачаны."

# ---------------------------------------------------------
# ИТОГИ
# ---------------------------------------------------------
echo ""
echo "========================================"
echo "🎉 СКАЧИВАНИЕ УСПЕШНО ЗАВЕРШЕНО!"
echo "========================================"
echo "📂 Путь: $TARGET_DIR"
echo ""
echo "📊 Проверка скачанных весов (первые 5):"
ls -lh model-*.safetensors | head -5
echo "..."
echo "📊 Проверка скачанных весов (последние 5):"
ls -lh model-*.safetensors | tail -5
echo ""
echo "📋 Все файлы в папке:"
ls -lh