#!/usr/bin/env bash
# download_llama3.2_90b_vision.sh - Скачивание meta-llama/Llama-3.2-90B-Vision-Instruct
# Включает поддержку токена для gated моделей

set -euo pipefail

# --- НАСТРОЙКИ ---
MODEL_ID="meta-llama/Llama-3.2-90B-Vision-Instruct"
# Папка для сохранения (убедитесь, что есть ~180 ГБ места)
TARGET_DIR="/mnt/nvme/huggingface/Llama-3.2-90B-Vision-Instruct"

# ВАЖНО: Если модель недоступна (ошибка 403), вставьте сюда ваш HF Token:
# Пример: HF_TOKEN="hf_xxxxxxxxxxxxxxxxxxxxxxx"
HF_TOKEN=""

echo "🚀 Скачиваем $MODEL_ID"
echo "📂 Папка назначения: $TARGET_DIR"
echo "⚠️  Модель состоит из 37 частей, общий размер ~175 ГБ."

# Создаем папку
mkdir -p "$TARGET_DIR"
cd "$TARGET_DIR"

# Функция для скачивания с поддержкой токена и докачки
do_download() {
    local url="$1"
    local filename="$2"
    
    echo "📥 Скачиваю: $filename"
    
    # Базовые параметры wget
    WGET_CMD="wget -c --timeout=3600 --tries=3 --continue --user-agent='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'"
    
    # Если токен указан, добавляем заголовок авторизации
    if [[ -n "$HF_TOKEN" ]]; then
        WGET_CMD="$WGET_CMD --header='Authorization: Bearer $HF_TOKEN'"
    fi
    
    # Выполняем команду
    eval $WGET_CMD -O "$filename" "$url" || {
        echo "❌ Ошибка скачивания $filename."
        if [[ -z "$HF_TOKEN" ]]; then
            echo "💡 Попробуйте добавить HF_TOKEN в начало скрипта. Модель может быть закрытой (gated)."
        fi
        return 1
    }
}

# ---------------------------------------------------------
# ЧАСТЬ 1: Скачивание весов (37 файлов safetensors)
# ---------------------------------------------------------
echo "========================================"
echo "⬇️  ЧАСТЬ 1: Веса модели (37 файлов)..."
echo "========================================"

# Цикл от 001 до 037 (Llama 3.2 разбита на 37 частей)
for i in {001..037}; do
    # Формируем имя файла: model-00001-of-00037.safetensors
    FILE_NAME=$(printf "model-%05d-of-00037.safetensors" "$i")
    FILE_URL="https://huggingface.co/$MODEL_ID/resolve/main/$FILE_NAME"
    
    do_download "$FILE_URL" "$FILE_NAME"
done

echo "✅ Часть 1 завершена: Все веса скачаны."

# ---------------------------------------------------------
# ЧАСТЬ 2: Скачивание конфигурационных и служебных файлов
# ---------------------------------------------------------
echo "========================================"
echo "⬇️  ЧАСТЬ 2: Конфигурации, Токенизатор, Vision..."
echo "========================================"

# Список всех нужных файлов (взято из вашего сообщения)
CONFIG_FILES=(
    "config.json"
    "generation_config.json"
    "model.safetensors.index.json"
    "preprocessor_config.json" # Важно для Vision компонента
    "tokenizer.json"
    "tokenizer_config.json"
    "special_tokens_map.json"
    "chat_template.json"
    "README.md"
    "LICENSE.txt"
    "USE_POLICY.md"
    ".gitattributes"
)

for file in "${CONFIG_FILES[@]}"; do
    FILE_URL="https://huggingface.co/$MODEL_ID/resolve/main/$file"
    do_download "$FILE_URL" "$file"
done

echo "✅ Часть 2 завершена: Конфигурации скачаны."

# ---------------------------------------------------------
# ИТОГИ
# ---------------------------------------------------------
echo ""
echo "========================================"
echo "🎉 СКАЧИВАНИЕ УСПЕШНО ЗАВЕРШЕНО!"
echo "========================================"
echo "📂 Путь: $TARGET_DIR"
echo ""
echo "📊 Проверка скачанных весов (первые 5):"
ls -lh model-*.safetensors | head -5
echo "..."
echo "📊 Проверка скачанных весов (последние 5):"
ls -lh model-*.safetensors | tail -5
echo ""
echo "📋 Все файлы в папке:"
ls -lh