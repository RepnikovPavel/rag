...
def last_token_pool(last_hidden_states: Tensor,
    ...
    """
    если был хотябы один документ хотя бы с одним токеном внтури то torch.Size([1, 1, 4096])
    если был хотябы один документ с несколькими N токенами внтури то torch.Size([1, N, 4096])
    если было несколько M документов с разным числом токенов то torch.Size([M, max(N_vec), 4096])
    """
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        print(f'outputs.last_hidden_state.size()',outputs.last_hidden_state.size())
    ...
    for run_idx in tqdm(range(N_runs),desc='perf test num_layers_in_buffer=18'):
        with torch.no_grad():
            ...
            encodings=batch_dict.encodings
            print('input_ids.size()',input_ids.size())
            if torch.prod(torch.tensor(input_ids.size()))>19458:
                num_layers_in_buffer = 4
            
            outputs = model.iter_forward_gpu_buff(
                input_ids=input_ids,
                attention_mask=attention_mask,
                use_cache=False,
                num_layers_in_buffer=num_layers_in_buffer
            )
            torch.cuda.synchronize()
